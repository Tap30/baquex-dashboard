...
export abstract class AuthStrategy {
   ...
   * identity provider (e.g., OIDC) or a direct API call.
   *
   * @returns A promise that resolves when the login is complete.
   */
  public abstract signin(params?: SigninParams): Promise<void>;
  ...
  /**
   * Attempts to refresh the user's access token silently.
   *
   * @returns A promise that resolves with the refreshed user object, or null if refresh fails.
   */
  public abstract refreshToken(): Promise<AuthenticatedUser | null>;
}
